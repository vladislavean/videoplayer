...
async def add_archive(url: str, camera_id: uuid.UUID):
    async with get_async_session() as session:
        ...
        return await insert_one(session=session, model=ArchivesTask, url=url, cameraId=camera_id)


@admin_archives_router.put("/update/{id}", summary="Обновление архива")
async def update_archive(id: uuid.UUID, url: str, camera_id: uuid.UUID):
    async with get_async_session() as session:
        camera_id = await find_one_or_none(session=session, model=Cameras, id=camera_id)
        if not camera_id:
            raise HTTPException(status_code=404, detail="Камера с таким id не существует")
        return await update_one(session=session, model=ArchivesTask, id=id, url=url)
